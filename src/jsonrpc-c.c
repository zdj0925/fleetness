...
static int invoke_procedure (struct jrpc_server *server, struct jrpc_connection *conn, char *name, cJSON * params,
                             cJSON * id)
{
    cJSON *returned = NULL;
...
static int eval_request (struct jrpc_server *server, struct jrpc_connection *conn, cJSON * root)
{
    ...
    if (method != NULL && method->type == cJSON_String)
    {
        ...
        if (params == NULL || params->type == cJSON_Array || params->type == cJSON_Object)
        {
            ...
            {
                // We have to copy ID because using it on the reply and deleting the response
                // Object will also delete ID
                cJSON *id_copy = NULL;
